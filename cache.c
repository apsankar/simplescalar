       ...
       unsigned int hit_latency)  /* latency in cycles for a hit */
{
 ...
 // cp->BIP_Counter = 0; //BIP_Counter resets at 32. (5 bits.)
 // cp->Policy_Sel = 512; // If Policy_Sel is 0 or +ve, it means Followers use BIP; else, LRU. 
  //In keeping with the reference paper, Policy_Sel saturates at 511 and -512 at both sides (10 bits.)
  ...
  for (bindex=0,i=0; i<nsets; i++)
    {
      ...
      if (cp->policy == DIP) {
        if (i % (nsets/32) == 0) cp->sets[i].Type = LRU_Dedicated;
        else if ((i+1) % (nsets/32) == 0) cp->sets[i].Type = BIP_Dedicated;
        else cp->sets[i].Type = Follower;
      }
    ...
    struct stat_sdb_t *sdb) /* stats database */
{
  ...
  if (!cp->name || !cp->name[0])
    ...
    name = cp->name;
 // sprintf(buf, "%s.Policy_Sel",name);
 // stat_reg_counter(sdb,buf, "Value of PSEL",Policy_Sel,0,NULL);
      ...
      FILE *stream)   /* output stream */
{
  ...
  fprintf(stream,
    "cache: %s: %.0f read_hits %.0f hits %.0f misses %.0f repls %.0f invalidations\n",
    cp->name, (double)cp->read_hits, (double)cp->hits, (double)cp->misses,
       ...
       md_addr_t *repl_addr)  /* for address of replaced block */
{
  ...
  case DIP:
    repl = cp->sets[set].way_tail;
    static int Policy_Sel=512;
    unsigned long int rand_int;
    rand_int = rand();
    if (cp->sets[set].Type == LRU_Dedicated) {
      update_way_list(&cp->sets[set], repl, Head);
      if (Policy_Sel < 1023) Policy_Sel++; //Typical LRU Policy.
    }
    else if (cp->sets[set].Type == BIP_Dedicated) {
     // if (cp->BIP_Counter == 0)
      if (rand_int % 32 == 0)  update_way_list(&cp->sets[set], repl, Head); //MRU Insertion.
      else  update_way_list(&cp->sets[set], repl, Tail);
      if (Policy_Sel > 0) Policy_Sel--; //LRU Insertion.
    }
    else {
      if (Policy_Sel >= 512) {  //Then Follower sets use BIP.
      //  if (cp->BIP_Counter == 0)
        if (rand_int % 32 == 0) update_way_list(&cp->sets[set], repl, Head); //MRU Insertion
        else update_way_list(&cp->sets[set], repl, Tail); //LRU Insertion.
      }
      else //If Policy_Sel < 0, that means BIP performs poorly; use LRU.
          update_way_list(&cp->sets[set], repl, Head);
    }
    break;
    // Have to increment BIP_Counter. Where to?

  case BIP:
    repl = cp->sets[set].way_tail;
    static long int counter_bip = 0;
    counter_bip++;
    if (counter_bip%32 == 0)
      {
     // counter_bip = 0;
  ...
  *repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
  ...
  { 
    ...
    lat += cp->blk_access_fn(Write,
           ...
           cp->bsize, repl, now+lat, NULL/*CACHE_MK_BADDR(cp, repl->tag, set)*/); // What's going on here??
  ...
  lat += cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize,
         repl, now+lat, NULL/*CACHE_MK_BADDR(cp, repl->tag, set)*/);
 ...
 cache_hit: /* slow hit handler */
  ...
  /* if LRU replacement or BIP or DIP and this is not the first element of list, reorder */
  if (blk->way_prev && (cp->policy == LRU || cp->policy == BIP || cp->policy == DIP))
    {
...
/* access a cache, perform a CMD operation on cache CP at address ADDR,
   places NBYTES of data at *P, returns latency of operation if initiated   //victim cache impl
   at NOW, places pointer to block user data in *UDATA, *P is untouched if
   cache blocks are not allocated (!CP->BALLOC), UDATA should be NULL if no
   user data is attached to blocks */
unsigned int        /* latency of access in cycles */
       ...
       md_addr_t rep_addr)
{
  ...
  *repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
  ...
  {  //The VICTIM CACHE only writes back.
    ...
    lat += cp->blk_access_fn(Write,
           CACHE_MK_BADDR(cp, repl->tag, set),
           cp->bsize, repl, now+lat, /*CACHE_MK_BADDR(cp, repl->tag, set)*/NULL);
    //6 parameters? But Vic_Access_fn takes only 5 parameters...
  }
    }
